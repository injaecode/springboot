...
class SbbApplicationTests {
	...
	/*
	@Autowired		//객체 자동 주입, JPA 메소드 사용 (findAll(), findById(), save(), delete())
	...
	public void testAnswerjpa() {
		...
		Question q=op.get();
		//2. Answer엔티티 테이블의 객체를 생성하고, setter를 사용해서 값을 저장
		//		setQuestion(q)
		Answer a = new Answer();
		a.setQuestion(q);
		a.setContent("수정된 내용에 대한 답변입니다");
	...
	*/
	...
	public void testjpa6() {
		//1. 테이블에서 삭제할 레코드를 가져옴
		Optional<Question> op=
		this.questionRepository.findById(0);
		//2. Optional에 저장된 객체를 꺼내옴
		Question q = op.get();
		//3. delete(q)
	...
	*/
	...
	public void testjpa5() {
		//1. 수정할 객체를 findBId() 메소드를 사용해서 가지고 온다.
		Optional<Question> op = this.questionRepository.findById(1);
		//2. 가져온 객체를 꺼내와 setter를 사용해서 수정
		Question q = op.get();
				q.setSubject("수정된 제목");
				q.setContent("수정된 내용");
		//3. 수정된 객체를 save(q)
	...
	*/
	...
	public void testjpa4() {
		List<Question> all = 
				this.questionRepository.findAllByOrderByCreateDataAsc();
		
		List<Question> all2 = 
				this.questionRepository.findAllByOrderByCreateDataDesc();
